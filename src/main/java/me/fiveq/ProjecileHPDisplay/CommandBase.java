...
public abstract class CommandBase extends BukkitCommand implements CommandExecutor {
    ...
    public boolean execute(CommandSender sender, String alias, String[] arguments) {
        ...
        if (playerOnly && !(sender instanceof Player)) {
            ColorMSG.send(sender, "&cOnly players can use this command.");
            return true;
        }

        String permission = getPermission();
        if (permission != null && !sender.hasPermission(permission)) {
            ColorMSG.send(sender, "&cYou don't have permission to use this command.");
            return true;
        }
        ...
        if (delayedPlayers != null && sender instanceof Player) {
            ...
            if (delayedPlayers.contains((player.getName()))) {
                ColorMSG.send(player, "&cPlease wait before using this command again.");
                return true;
            }

            delayedPlayers.add(player.getName());
            Bukkit.getScheduler().scheduleSyncDelayedTask(ProjectileHpDisplayer.getInstance(), () -> {
